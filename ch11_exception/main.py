...
# table.add_row(("6","ModuleNotFoundError","import할 모듈이 없을때"))
...
#     print(e)
'''
이상은 특정 예외가 아니라 194 번으로 넘어가기만 하면 바로 예외가 발생해버립니다.
...
'''
# class NegativeAgeError(Exception): # Excption 클래스를 상속받았다는 의미
...
ScoreOutOfRangeError 클래스를 정의해서 사용하겠스빈다.

입력 받는데 예를 들어 백점이라고 입력하면 '점수는 숫자로 입력해야합니다'라는 메시지를 출력하세요.
실수로 입력하면 '정수로 입력해야 합니다'라는 메시지를 출력하세요.

예상할 수 없는 예외가 발생시 Exception을 적용하여 디볼트 에러 메시지를 출력하세요.
...
#     if score > 100:
#         print('점수는 0이상 100이하입니다')
#         raise ScoreOutOfRangeError
#     if score < 0:
#         print('점수는 0이상 100이하입니다')
#         raise NegativeScoreRange
#     score = int(score)
...
7. 프로그램 종료 메시지를 출력한다.
8. 마이너스인덱스는 적용시키지 않는다. -> 사용자 정의 예외 클래스를 통해서 적용합니다. -> NegaviceNumIndexError라고 이름 짓고 처리하겠습니다.
'''
...
#     pass
# try:
#     li1 = int(input('인덱스 넘버를 입력하시오.>>>'))
#     if li1 <0 :
#         print('마이너스인덱스는 적용시키지 않는다.')
#         raise NegativeNumIndexError
#     print(f'인덱스 값은 {my_list[li1]}')
...
# 클래스 변수인 school이 전역이 아니므로 main단계에서 선언되지 않은 것으로 간주됨.-> 그래서 NameError가 발생했지 속성명과는 관계가 없음.
# try:
#     attr_name = input('속성명을 입력하시오.>>>')
