...
'''
# print(scores.pop())# 근데 .pop() call3() 유형입니다. 즉 return값이 있는데, 그 삭제한 element가 return되기 때문에 print(scores.pop())은 현재 scores의 맨마지막 element인 100이 콘솔에 출력 됩니다.
# print(scores.pop(0))
'''
교재에 없는 삭제 메서드 : .remove(값)을 사용하면 list 내에 해당 값을 찾아 삭제함(그러니까 argument로 인데스 넘버를 요구하는게 아니라 특정 데이터를 요구한다고 볼 수 있습니다.)
'''
...
일반 for문 형식으로 한번
enhanced for문 형식으로 한번해서 첫 번째 element가 4가 되어야겠습니다.
'''
# for i in range(len(li4)):
#     new_element = li4[i]*2
            # 이거 element를 뽑아서 2를 곱한거지 li4를 바꾼게 아니라서 그렇습니다.
#     li4[i] = new_element
# print(li4)
#
...
#     n+=1
# print(li4)
# # 사실상 list의 element 들에게 동일한 연산을 일괄적으로 적용하는 method가 있기 때문에 추후 설명할 예정입니다. 백엔드에서보다 프론트에서 더 많이 쓰여서 예시는 이후 수업
# '''
#     2. tuple 튜플
...
'''
# tu6 = 'hello.','good morning','my name is', 'kim-il',' am','20','years old'
# for word in tu6:
#     print(word.title(), end='')
#     # 결과값 : Hello.Good MorningMy Name IsKim-IlI Am20Years Old
#
# print()
# print(tu6)
'''
...
'''
    se = {} 형태로 비어있는 set를 생성했을 경우 se는 사실 <class 'dict'>로 나옵니다. 아직 배우지 않은 dictionary 자료형으로 생성됩니다. 그래서 비어있는 set를 생성하기 위해서는 반드시 # 2 방식으로 만들어줘야합니다.
...
#
# # remove() vs. discard()
# # se3.remove(70)        # 오류 발생 - '값을 정확하게 입력'해야만 한다고 했음.
# se3.discard(70)         # 얘는 오류 발생 안합니다. discard()는 element로 정확한 값이 없으면 그냥 종료
# print(se3)
...
# print(li001[2:7].pop(1))
# # 아래 코드 라인에 주목하시면 됩니다.
# print(f'3 번째 요소로부터 7번째 요소 중 2 번째 요소 ={li001[2:7].pop(1)}') # {li001[2:7][1]}
'''
...
# print(f'{month}월은 {last_date_dict[month]}일 까지입니다.')
# # #2 : list를 이용하는데, [31, 28, 31, 30, 31, 30, 31, 30, 31, 30, 31]을 이용하는 방법
# num = int(input('1 ~ 12 사이의 월을 입력하세요 >>>'))
# li1 = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
# for i in range(num-1):
#     print(li1[i+1])
#     print(f'{num}월은 {li1[i+1]}일 까지입니다.')
...
#     li2_date = 'x'
#
# print(f'{month2}월은 {li2_date} 까지입니다)

'''
이상의 코드 라인에서 중요한 것은 in 개념입니다.
in 뒤에는 다양한 것들이 올 수 있는데, 특히 반복가능 객체(iterable)이 올 수 있다는 점입니다.
그래서
elif month2 in [1,3,5,7,8,10,12]:
    li2_date = li2[2]
...
응용 예제
수학 여행을 어디로 갈 지 결정하기 위해 학생들이 희망하는 모든 수학 여행 장소를 조사하기로 했습니다. 학생들이 원하는 장소를 입력 받아 동일한 입력을 무시하고 모든 입력을 저장하려고 합니다.
...
4번째 숫자를 입력하세요 >>> 25
5번째 숫자를 입력하세요 >>> 30
입력 받은 숫자는 [5, 10, 15, 20, 30]입니다.
입력 받은 숫자는 숫자들 중 짝수는 [10, 20, 30]입니다.
'''
# li1 = []
# li2 = []
# li_num = int(input('몇 개의 숫자를 입력할까요?>>>'))
# for n in range(5):
#     n += 1
#     number = int(input(f'{n}번째 숫자를 입력하세요 >>>'))
#     li1.append(number)
#     # 이 number이 짝수면 li2에 대입하면 되겠네요.
#     if number % 2 == 0:
#         li2.append(number)
# print(f'입력 받은 숫자는 {li1}입니다.')
# print(f'입력 받은 숫자는 숫자들 중 짝수는 {li2}')
'''
딕셔너리 기반의 연락처 관리
...
# dict1 = []
# for i in range(3):
#     li1 = input(f'{i+1} 번째 사람의 이름의 입력하세요 >>>')
#     li2 = input(f'{i+1} 번째 사람의 연락처를 입력하세요 >>>')
#     dict1[li1] = li2
...
#         numbers2.append(i+1)
#     print(numbers2)
#     # 이거 쓰면 밑에 함수 호출하거나 구현부를 작성하지 않아도 오류가 나지않음
...
#     print(temp_list)
# # 함수 호출 영역
# add_numbers3(last_num,hello)    # 결과값 :
# 1 다수의 수강생
'''
짝수와 홀수의 개수 세기
