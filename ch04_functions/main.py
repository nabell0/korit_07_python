...
# display_name_age(23,'김삼') #keyword argument
'''
우리가 예를 들어 input('이름을 입력하세요 >>>')을 이용해서 이것을 name이라는 변수에 담았따고 가정하면,
name=input('이름을 입력하세요 >>>')이라고 작성해왔습니다.

그러면 여태까지 input()이라는 파이썬 내장 함수를 사용하고 있었다고 볼 수 있습니다. 파이썬 내장 함수는 이미 정의가 되었고,개발자들은 함수 호출만 잘하면 되겠네요.
사용자 정의 함수의 경우 개발자 자신이 함수를 정의하고, 그 후에 호출까지 하는 것까지의 과정이라고 보시면 되겠습니다.

내장 함수 예시
print()/type()/int()/float()/input()

2. 메서드(methods) : 특정 객체가 가지고 있는 함수를 의미. 특정 자료형에ㅐ 포함되어있는 함수. 사실 함수와 메서드는 동일한 개념이지만, 호출 방식에 있어서의 차이가 있습니다.
...
57번 라인의 결과값과 59번 라인의 결과값이 차이가 생겼음.
...
그럼 현재 multiyply를 call2() 유형으로 정의했다고 볼수 있음.
