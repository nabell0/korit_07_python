...
while not end_of_game:
    ...
    #     print(stages[lives])
    #     end_of_game = True
        # 라고 작성하시면 안됩니다. 반복문 내부에서 guess가 일치하는지 여부를 따지는 중입니다. 예를 들어 chosen_word가 apple이고, guess가 a라고 가정했을 때, 첫 번째 반복에서는 display의 0번지가 a로 바뀝니다. 그런데 반복문 내부에 위치해 있기 때문에 1,2,3,4번지에 대해서도 a가 display의 인덱스와 인치하는 element인지를 확인하게 됩니다. 그 결과 p p l e가 a와 다르기 때문에 lives -= 1이 4번 적용되어서 맞는 답을 입력했음에도 불구하고 행맨이 완성되는 것을 확인할 수 있게 됩니다.
    ...
    if '_' not in display:
        print('정답입니다 !!')
        end_of_game = True    # 이 시점에 end_of_game이 True가 되었으므로 다음 반복문이 실행되지 않음 -> 109번 코드라인이 실행된다는 것을 의미합니다.
        #break       # 바로 반복문 정지 -> 105번 코드라인이 실행 x
        print(f'정답은 {chosen_word}입니다.')

    # 현재 상황이 콘솔창에 출력돼서 user에게 안내가 가면 좋겠네요
    print(' '.join(display))
# todo - 4 : lives의 변수와 stages 리스트의 관계를 파악하여 guess를 입력할 때 마다 올바른 stages의 element가 출력될 수 있도록 작성하세요.
