...
# getter의 호출 예시 객체명.속성명 -> 객체명.메서드명()이 아니라는 것에 주목
...
다만 더 신기한 것은 것은 객체명.속성명 뒤에 ()가 없음에도 불구하고 그냥 파이썬은 이걸 메서드처럼 처리해준다는 겁니다. 그래서 '객체명.속성명'이면 getter로 처리해주고, '객체명.속성명 = 데이터'면 setter로 처리해준다고 보시면 됩니다.

이상의 코드라인이 성립하기 위해서 필수적인 부분이
'@property'와 '@속성명.setter'라는 '데코레이터(decorator)'때문입니다.

그래서 이거 원래는 자동 생성되기 때문에 일일이 에너테이션 달고 _속성명인지 그냥 속성명인지 따질 필요가 없는데 지금은 좀 쓸모가 없는 상황입니다.

저희는 python으로 백엔드를 짜지 않을거기 때문에 일단 이런 방식으로 setter / getter를 생성한다는 점만 유의해두시고 springboot에 대비하여 Java방식의 setter/getter를 작성 하겠습니다.
'''