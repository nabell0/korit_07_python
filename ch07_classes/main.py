...
print(waffle)   # 결과값 : <__main__.WaffleMachine object at 0x00000189273552B0>

'''
클래스의 구성

1. 클래스의 기본 구성
    객체를 만들어내는 클래스는 객체가 가져야 할 구성 요소를 지닙니다(Java 때 제가 방이 가져야 할 구성 요소는 무엇이었냐고 질문했었습니다)
    ...
    클래스를 구성하는 속성은 두 가지로 구분됩니다.
        1) 클래스 변수 : 클래스를 기반으로 생성된 모든 인스턴스들이 공유하는 변수 (Java에서는 얘를 static 변수라고 했습니다.ㅣ)
    ...
    메서드는 특징에 따라서
        ...
        2) 정적 메서드
        3) 인스턴스 메서드
    입니다. Java에서 정적 메서드라고 하던게 클래스 메서드에 해당되고, 정적 메서드는 또 따로 있다고 볼 수도 있고 Java의 정적 메서드가 파이썬의 클래스메서드 + 정적메서드라고 볼 수도 있습니다.
...
# display_info2()(call3()유형으로 작성)를 정의하여 다음 실행 예와 같이 출력되도록 작성.
# 실행 예
# 제 이름은 ___이고, ____살입니다.
# 연락처는 _____인데,___에 살고 있습니다.
# 코드 실행
# print(person02.set_info())
