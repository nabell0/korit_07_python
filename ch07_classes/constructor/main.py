...
그럼 Java에서 수업한 것 처럼 속성값이 대입되지 않은 객체를 생성한 다음에 속성값을 집어넣어주는 과정을 거쳐야 합니다.
...
# 인스턴스가 소멸되었습니다.
# emily is dead
# 라고 결과값이 나옵니다. 이 이유는 모든 코드블록이 다 읽어지면 메모리에 할당된 객체는 자동 소멸하기 때문입니다.
# 그래서 강제로 emily is dead. 를 출력하고 싶지 않다면 꼼수가 필요함
# '''
...
getter만을 활용하여,
김일 학생의 나이는 20 살로, 파이썬 과목의 점수는 4.5점입니다. 라고 출력.
'''
class Student1:
    def set_name(self,name):
        self.name = name
    ...
    def set_age(self,age):
        if age < 0 or age > 200:
            print('불가능한 입력입니다.')
            return          # 메서드에 return 하고 비워두면 메서드 종료
        self.age = age
    def get_age(self):
        return self.age
...
'''
그렇다면 Java를 기준으로 봣을 때 setter 내부에는 비지니스 로직이 들어갈 수 있었습니다.
완전 동일하게 할겁니다.
...
여기서 생기는 의문점은 그겁니다. -> 아니 매개변수 생성자를 통해서 생성했는데 객체 생성 시점에 -102살 입력하면 되는거 아니냐 할 수 있는데 추후 설명.
'''
