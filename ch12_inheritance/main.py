...
2. 서브 클래스 Hybrid는 다음과 같은 특징을 지니고 있습니다.
    1) 최대 배터리 충전량은 30
    2) 배터리를 충전하는 charge() 메서드가 존재합니다. 최대 충전량을 초과할 수 없고, 0보다 작은 값으로 충전할 수 없습니다.
...
#             return          # 메서드 종료
#         self.oil += oil     # 이상의 조건문이 실행되지 않으면 속서에다가 oil만큼 더해줍니다.
...
#     # 고유 메서드를 정의했는데 부모 메서드 가지고 오고 추가함.
#     # 그냥 car_info() override해서 재정의했을 것 같습니다.
